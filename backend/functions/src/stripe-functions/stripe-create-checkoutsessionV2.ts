...
import { stripe, createCustomerRecord } from "./common";
...
// console.log("config.customersCollectionPath", config.customersCollectionPath);
logs.info("config.customersCollectionPath", config.customersCollectionPath);
/**
 * Create a CheckoutSession or PaymentIntent based on which client is being used.
 * trigger:  `/${config.customersCollectionPath}/{uid}/checkout_sessions/{id}`
 */
export const createCheckoutSessionV2 = onDocumentCreated(
  // `/${config.customersCollectionPath}/{uid}/checkout_sessions/{id}`,
  `/stripe-customers/{uid}/checkout_sessions/{id}`,

  async (event) => {
    const snap = event.data;

    if (!snap) {
      logs.checkoutSessionCreationError(event.params.id, new Error("No data"));
      return;
    }
    ...
    try {
      logs.creatingCheckoutSession(event.params.id);
      ...
      if (client === "web") {
        ...
        const session = await stripe.checkout.sessions.create(
          sessionCreateParams,
          { idempotencyKey: event.params.id }
        );
        await snap.ref.set(
          {
            client,
        ...
        if (mode === "payment") {
          ...
          const subscription = await stripe.subscriptions.create({
            ...
            metadata: {
              firebaseUserUID: event.params.id,
            },
          });
      ...
      logs.checkoutSessionCreated(event.params.id);
      return;
    } catch (error) {
      logs.checkoutSessionCreationError(event.params.id, error as Error);
      await snap.ref.set(
